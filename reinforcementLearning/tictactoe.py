...
def play_move(state, player, block_num):
    if state[int((block_num-1)/3)][(block_num-1)%3] is ' ':
        ...
        block_num = int(input("La casilla no está vacia, elige de nuevo"))
...
def getBestMove(state, player):
    ...
    for empty_cell in empty_cells:
        ...
        if player == 'O':    # If AI
            result = getBestMove(new_state, 'O')    # make more depth tree for human
            move['score'] = result
        else:
            result = getBestMove(new_state, 'X')    # make more depth tree for AI
            move['score'] = result
...
while play_again == 'Y' or play_again == 'y':
    ...
    player_choice = input("Elige quien empieza - X (Tu) o O(AI): ")
    winner = None
    ...
    while current_state == "Not Done":
        if current_player_idx == 0: # Human's turn
            block_choice = int(input("Tu turno, elige entre 1 a 9: "))
            play_move(game_state ,players[current_player_idx], block_choice)
        else:   # AI's turn
            block_choice = getBestMove(game_state, players[current_player_idx])
            play_move(game_state ,players[current_player_idx], block_choice)
            print("AI mueve: " + str(block_choice))
        print_board(game_state)
        winner, current_state = check_current_state(game_state)
        if winner is not None:
            print(str(winner) + " Gané!")
        else:
            current_player_idx = (current_player_idx + 1)%2
        
        if current_state is "Nulo":
            print("Nulo!")
